      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant={isBillPayment ? "secondary" : transaction.type === "income" ? "default" : "destructive"}>
              {isBillPayment ? "Pagamento de Fatura" : transaction.type === "income" ? "Entrada" : "Saída"}
            </Badge>
          </div>
          <p className={`text-2xl font-bold ${isBillPayment ? "text-muted-foreground" : transaction.type === "income" ? "text-income" : "text-expense"}`}>
            {isBillPayment ? fmt(transaction.amount) : `${transaction.type === "income" ? "+" : "-"}${fmt(transaction.amount)}`}
          </p>
